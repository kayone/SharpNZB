...
namespace SharpNzb
{
    class IISController
    {
        ...
        internal static Process StartIIS()
        {
            ...
            Environment.SetEnvironmentVariable("SHARPNZB_PATH", Config.ProjectRoot);
        ...
        private static void UpdateIISConfig()
        {
            ...
            var bindings = configXml.XPathSelectElement("configuration/system.applicationHost/sites").Elements("site").Where(d => d.Attribute("name").Value.ToLowerInvariant() == "sharpnzb").First().Element("bindings");
